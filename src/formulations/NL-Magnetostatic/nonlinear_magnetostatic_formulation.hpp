...
class NLMagnetostaticFormulation : public hephaestus::NLStaticsFormulation
{
public:
  NLMagnetostaticFormulation(const std::string & magnetic_reluctivity_name,
                             std::string magnetic_permeability_name,
                             const std::string & magnetic_vector_potential_name);

  ~NLMagnetostaticFormulation() override = default;
...
protected:
  ...
  const std::string & _magnetic_reluctivity_name =
      hephaestus::NLStaticsFormulation::_alpha_coef_name;
};
} // namespace hephaestus
